...
def clear(df,col,val):
    ...
    """
    val_count=df[col].value_counts(val)
    return val_count
...
def instances_based_condition(df,col1,val1,col2,val2):
    ...
    """
    instance=weather[(weather["Wind Spd (km/h)"] > 35) & (weather["Visibility (km)"] == 25)]
    #instance=np.logical_and(df[col1] > val1 , df[col2] == val2)
    #instance=[(df[col1] > val1).all() & (df[col2] == val2).all()]
    return instance
...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    aggregated_value=pd.pivot_table(data=df,index=date_col,values=[agg_col],aggfunc=agg)
...
agg_values_ina_month(weather,'Date/Time','Temp (C)',[np.mean, max, min, sum, len] )
